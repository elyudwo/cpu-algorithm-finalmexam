...
머신러닝에 자주 이용되는 파이썬이 아닌 c++로 코드를 구현함에 있어 스스로 코드를 짜느라 쉽지 않았지만 기존의 bfs,dfs,그리디,크루스칼과 같은 기본 알고리즘만 접하던 시야에서 벗어나 좀 더 거시적인 관점에서 프로그래밍을 보게되었습니다.

교수님께서 알려주신 설명에 근거해 머릿속에서 생각을 정리한뒤 그것을 코드로 옮기는 작업이 신기했고 알고리즘에서도 창의적인 생각이 필요하다는 사실을 알게되었습니다.
